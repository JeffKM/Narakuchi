...
## Phase 8 — 다마고찌화: 펫 육성 + 돌봄 리텐션 (데모 검증 이후)

> 합의 2026-06-21(grill-with-docs → 메모리 `tamagotchi-evolution-plan`, 결정 → **ADR 0005**). 데모를 *카페 방문자가 매일 여는 앱*으로 키운다. 미연시는 별개 트랙.
> 설계 단일 출처는 **ADR 0005**. 아래는 코드 슬라이스 골격(수치는 전부 `data/balance` 튜닝거리로 미결).

**원칙(ADR 0005 요약)**
- **역할 분담**: 메인 = 호감도(관계·수집), 펫 = 육성(키우는 재미). **두 트랙 따로** — 메인은 *하루 한 명* 선택, 펫은 *활성 1마리* 의무.
- **돌봄 압박(되돌릴 수 있는)**: 🟢 표면(기분·배고픔·정돈)에만 작용, 🔴(보유 체키·관계 단계·누적 호감도·자란 성장단계)는 불가침. 컨디션=성장 *속도*만, 역행 0.
- **분기형 진화**: 같은 아기 → 간식/놀기 *밸런스*로 마른/보통/뚱뚱 분기 → 성체에서 **확정·영구**. 특수형 분기는 최후.
- **체키 소스**: 메인 게이지→메인 체키 / 펫 게이지→펫 체키(진척 일정량마다). **성장 단계 = 진화 연출**(체키 아님). 데모=무한 연타, 실제=게이트.
- **한 마리씩**: 성체 완성 → 졸업(카페 상주) → 다음 펫 해금. ♡(T39)로 확정 해금.
...
- [x] **D2** 체형 3분기 — **완료**: 성장 단계축(D1)에 **직교**로 체형축 신설. 누적 *간식 vs 놀기* 치우침(`lean`)이 **성체 도달 순간** 체형을 확정·영구(🔴 불가침). ① **`data/balance.gd`**: `PET_BODY_TYPES`(thin/normal/fat) + `pet_body_type(lean)` 단일 출처 — 간식 우세(lean>0)=통통·놀기 우세(lean<0)=마름·비등(0)=보통(쓰담 중립). ② **`Meters.grow_pet(character, kind)`**: kind(snack/play/pet)로 lean 누적(간식 +1·놀기 −1·쓰담 0) + 성체 경계서 `pet_body_type` 확정(이미 확정됐으면 보존 — 성체 후 lean 움직여도 body 불변) / `pet_body()` 게터. ③ **`Characters.pet_stage_prefix(id, stage, body)`**: 성체 체형 분기 — 마름(`_thin`)·통통(`_fat`)만 신규 아트, 보통/미확정=캐논 재사용(D0 16컷 활용). ④ **`SaveManager`**: 펫 블록에 `lean=0`·`body=""`(메인 제외·`_merge_defaults`/`_coerce_types`로 구세이브 무손상). ⑤ **`cafe`**: `_on_sion_action` 이 `grow_pet(pet, id)`로 kind 전달, `_build`/`swap_active`/`_on_pet_grew` 가 body 반영해 스프라이트 스왑(재방문/펫 교체 시 자란 체형 유지), 성체 안내 티커가 체형 노출("다 자랐어요! (통통)"). 디버그 키 9 순환은 이미 `_thin`/`_fat` 포함(정합). **검수**: 헤드리스 445통과(시스템 158 — 체형 판정·접두어·세이브·확정 영구성·격리 신규 18 / 콘텐츠 270 / 컷인 17), 부팅 무에러, 회귀 0. 화이트 플래시·reveal 연출은 D3.
- [x] **D3** 진화 연출 — **완료**: 단계 도달(D1)·체형 확정(D2) 순간에 **화이트 플래시 + 하드 스왑 + 반짝임**으로 진화 임팩트(감정 + 공유성). **추가 아트 0**(셰이더 없이 `_draw`+트윈). ① **`scripts/ui/pet_evolve_fx.gd`**(PetEvolveFx, Node2D): `play(swap)` 가 ⓐ 흰 광휘 원이 펫을 덮으며 팽창·밝아짐(절정) → ⓑ **그 순간 swap() 호출**(텍스처 하드 스왑 — 흰 빛 속에서 새 모습으로) → ⓒ 흰 빛 사그라들며 공개 + 골든 햇살(`BurstRays` 재사용·캔들 톤) + ★/♡ 반짝이 사방 분출(나비 승급 연출의 펫판) → ⓓ 자동 `queue_free`. 성체(분기 reveal)는 반경·여운 더 크게(R 60→84). ② **`cafe._on_pet_grew`**: 즉시 set_prefix 하던 것을 **연출이 절정에 삼켜 reveal**(`fx.play(swap=set_prefix+hop)`), `Sfx.event(&"stage_up_reveal")` 재사용(추가 에셋 0). ③ **검수**: 헤드리스 447통과(콘텐츠 272 — 진화 연출 스왑콜백·자동정리 스모크 2 신규 / 시스템 158 / 컷인 17), 부팅 무에러, 회귀 0. 흰 광휘·반짝이 시각 품질은 실기기 몫(헤드리스 렌더 없음). HUD 성장 미터는 D4.
- [x] **D4** HUD — **완료**: 펫일 때 상단 게이지를 "호감도"에서 **"성장 미터"로 용도 변경**(메인 옥자/메인 호감도 경로 무손상 — `Characters.is_main` 분기 그대로). ① **`data/balance.gd`**: `pet_stage_progress(care_count) → [cur, need]` 단일 출처 — 현재 단계 내 진척(다음 진화까지), 성체는 `[PER, PER]`(포화=가득, 비우지 않음). ② **`scripts/ui/hud.gd`**: 펫 분기를 누적 돌봄(`{pet}.growth`)에서 파생 — 게이지 채움=단계 진척, 색을 **`ACCENT_GREEN`(성장=초록, 호감도 레드와 의미 구분)** 으로 스왑(메인 복귀 시 `BLOOD`로 원복), 텍스트="이름 아기 3/8"(성장 중)·"이름 다 자람 (통통)"(성체=확정 체형 D2 노출, 보통/미확정은 생략). 단계·체형은 `Balance`(static)+`SaveManager`로 직접 파생(HUD는 표시만 — 미터 변경은 `Meters` 책임, 파일 계약 준수). `STAGE_LABEL`/`BODY_LABEL` 한글 라벨 상수(단계 문자열 단일 출처=Balance). ③ **저장 무변**(기존 `growth`/`body` 필드 재사용 — 새 스키마 0). **검수**: 헤드리스 452통과(시스템 163 — 단계 진척 경계/성체 포화 5 신규 / 콘텐츠 272 / 컷인 17), 회귀 0, 부팅 무에러. 실기기 시각(초록 미터·진척 채움)은 실기기 몫.

**🧩 코드 트랙 (수직 슬라이스 — 첫 펫 = 시온이)** — 데모 검증 *이후* 리텐션 풀세트

- [ ] **T40** 돌봄 압박 토대 — 🟢 컨디션(배고픔·기분) 상태 + 방치→나빠짐/들르면 복구, 🔴 불가침 가드. `Meters`/`SaveManager` 스키마(역행 없는 누적 성장 필드). (ADR 0005)
- [ ] **T41** 펫 육성 엔진(시온이) — 장기 성장미터(아기→유년→성체) + 간식/놀기 밸런스→체형 분기(성체 확정) + 컨디션=속도 결합(B). `data/balance` 수치 노출.
- [ ] **T42** 진화 연출 — 성장 단계 도달 시 진화 애니/장면(체키 아님, 희소 임팩트).
- [ ] **T43** 두 트랙 데일리 게이트 — 메인 *하루 한 명* 진척→체키→잠금 / 펫 독립 게이트→펫 체키. 데모/디버그 무한 연타 경로 보존.
- [ ] **T44** 졸업 + 한 마리씩 — 성체 완성→졸업(상주) + 다음 펫 해금. (T39 ♡ 경제와 결합)
