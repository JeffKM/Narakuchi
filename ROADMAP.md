...
| [#14](https://github.com/JeffKM/Narakuchi/issues/14) | 🧧 멜 메인 배선 (라이브·교감 파리티·반말 컷인·체키·탭) | AFK | #13 | 🔄 진행 — 선행 배선만(레지스트리·idle·체키 art) · 전용 대사·반말 컷인·탭 잠금 해제 잔여 |
...
- [~] 신규 메인 1명당: 표정 6장(idle/smile/shy/sad/brew/talk) + 초상 + 인트로(지뢰계) 체키 의상 1벌 → 이벤트 의상 점증 `(미호=#1)` — **미호·바나 ✅ 완료**(바나=뱀파이어 메이드 표정6·초상·지뢰계 의상 `bana_jirai`·체키 배경 `bg_cheki_bana_jirai`, 퍼플/네이비 램프), **멜 ✅ 아트 완료** `(멜=#13)`(강시 메이드=블루/틸 강시 관복·볼 빨간 점·관모·흑발 일자뱅, 표정 6종·초상·청운 에이드 brew·지뢰계 의상 `mel_jirai` + `bg_cheki_mel_jirai` 청록 야경 추가, 헤드리스 로드 검증. ⚠️ 검수 시 청록/틸 미드톤 1색 보강 후보. **메인 배선 #14는 잔여** — 전용 대사·반말 컷인·컬렉션 탭 잠금 해제)
