...
# ── 펫 육성 — 데모 진화 슬라이스 (D1, ADR 0005) ──────────────
# 간식·놀기·쓰담 3버튼이 '성장 카운트'를 올려(체키 제외 — 수집이지 육성 아님) 단계가 오른다.
# 데모 모드(DEMO)에서만 동작 — 돌봄 압박·하루 제한 없이 한 세션 완주(페이싱 A). 속도축(T40)은 데모에서 끔.
# 단계 = 아기→유년→성체(인덱스 0=시작). 성체 체형 분기(마름/보통/통통)는 D2(누적 간식 vs 놀기).
# 누적이라 역행 0(🔴 불가침). 스프라이트는 Characters.pet_stage_prefix 로 단계→접두어 파생.
...
# ── 펫 육성 단계 (데모 진화 슬라이스 D1) ──
## 누적 돌봄 횟수 → 현재 성장 단계 인덱스(0=아기 …). 마지막 단계에서 포화(더 안 오름).
static func pet_growth_stage_index(care_count: int) -> int:
  var idx := care_count / PET_GROWTH_PER_STAGE
  return mini(idx, PET_GROWTH_STAGES.size() - 1)


## 누적 돌봄 횟수 → 성장 단계 문자열("baby"|"child"|"adult"). (단계 판정 단일 출처)
static func pet_growth_stage(care_count: int) -> String:
  return String(PET_GROWTH_STAGES[pet_growth_stage_index(care_count)])
